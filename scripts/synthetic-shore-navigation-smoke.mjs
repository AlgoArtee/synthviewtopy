...
const position = (x, groundY, z) => page.evaluate(({ x, groundY, z }) => {
  ...
  shore.camera.position.set(x, groundY + 1.7, z);
  shore.camera.lookAt(x, groundY + 1.7, z + 100);
}, { x, groundY, z });
...
try {
  ...
  assert(Math.abs(ascent.position[1] - 9.7) < 0.025 && ascent.surface === 'anchor pier', 'Ascent must reach the 8 m deck continuously.');
  await move('a', 2100);
  const landing = await state();
  assert(landing.position[0] > 40 && landing.position[0] < 45, 'Landing must connect stairs to pier stem.');
  assert(Math.abs(landing.position[1] - 9.7) < 0.025, 'Crossing landing must retain deck height.');
  await move('s', 1500);
  ...
  assert(Math.abs(anchor.position[1] - 9.7) < 0.025, 'Anchor junction must not drop to ocean.');
  await move('d', 3000);
  ...
  assert(Math.abs(railing.position[1] - 9.7) < 0.025, 'Railing must prevent stepping into water.');
  ...
  assert(Math.abs(descent.position[1] - (descent.position[2] * 0.04 + 1.7)) < 0.025, 'Descent must end at the sand height.');
