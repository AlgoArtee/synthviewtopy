...
import { buildPharmacologyDistrict } from './pharmacologyDistrict';
...
import { buildGenomicsLabsDistrict } from './genomicsLabsDistrict';
import { buildProteomicsLabsDistrict } from './proteomicsLabsDistrict';
import { buildOmicsLabsDistrict } from './omicsLabsDistrict';
import { buildComputationalBiologyLabsDistrict } from './computationalBiologyLabsDistrict';
import { buildRoboticsLabsDistrict } from './roboticsLabsDistrict';
import { buildElectronicsLabsDistrict } from './electronicsLabsDistrict';
import { buildBiochemistryLabsDistrict } from './biochemistryLabsDistrict';
...
import { buildAstronomyAstrobiologyLabsDistrict } from './astronomyAstrobiologyLabsDistrict';
import { buildMaterialsScienceLabsDistrict } from './materialsScienceLabsDistrict';
import { buildEnvironmentalScienceLabsDistrict } from './environmentalScienceLabsDistrict';
import { buildToxicologyLabsDistrict } from './toxicologyLabsDistrict';
...
function buildCore(group: THREE.Group, definition: DistrictDefinition, height: number) {
  ...
  if (id === 'dark-center-lab-megabuilding') {
    // The north/south shared district delimiter passes through the integrated
    // core. Keep that civic axis visibly and physically open by splitting the
    // low podium into two solid wings; the tower layers begin high enough to
    // form a genuine covered passage rather than an invisible collision hole.
    const podiumWidth = width * 0.88;
    const passageWidth = Math.max(2.1, width * 0.16);
    const wingWidth = (podiumWidth - passageWidth) * 0.5;
    for (const [side, direction] of [['WEST', -1], ['EAST', 1]] as const) {
      const podiumWing = roundedBlock(
        id,
        wingWidth,
        1.1,
        depth * 0.9,
        materials.dark,
        direction * (passageWidth + wingWidth) * 0.5,
        0.32,
        0,
        0.36,
      );
      podiumWing.name = `${id}__${side}_PODIUM_WING`;
      group.add(podiumWing);
    }
    const delimiterPassage = prepareMesh(
      new THREE.Mesh(
        new THREE.BoxGeometry(passageWidth * 0.94, 0.012, depth * 0.94),
        materials.metal,
      ),
      id,
      false,
    );
    delimiterPassage.name = `${id}__PUBLIC_DELIMITER_PASSAGE`;
    // The split foundation exposes the existing ground-level delimiter road;
    // this thin inlay is only a visible continuation marker and stays within
    // ordinary WALK step height.
    delimiterPassage.position.set(0, 0.012, 0);
    delimiterPassage.userData.walkable = true;
    delimiterPassage.userData.navObstacle = false;
    delimiterPassage.userData.districtDelimiterAccess = true;
    group.add(delimiterPassage);
    const layers = [
...
function populateDistrictSectorCampus(group: THREE.Group, definition: DistrictDefinition, random: () => number) {
  ...
  if (definition.id === 'security') {
    group.traverse((child) => {
      if (!child.name.startsWith('SECURITY__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'pharmacology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('PHARMACOLOGY__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'toxicology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('TOXICOLOGY__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'microbiology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('MICROBIOLOGY__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'molecular-biology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('MOLECULAR__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'bioanalytics-lab') {
    group.traverse((child) => {
      if (!child.name.startsWith('BIOANALYTICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'genomics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('GENOMICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'proteomics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('PROTEOMICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'omics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('OMICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'computational-biology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('COMPBIO__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'robotics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('ROBOTICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'electronics-microelectronics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('ELECTRONICS__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'biochemistry-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('BIOCHEM__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'organic-chemistry-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('ORGCHEM__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'inorganic-chemistry') {
    group.traverse((child) => {
      if (!child.name.startsWith('INORGCHEM__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'particle-physics-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('PARTICLE__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'astronomy-astrobiology-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('ASTRO__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'materials-science-lab') {
    group.traverse((child) => {
      if (!child.name.startsWith('MATTER__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
  ...
  if (definition.id === 'environmental-science-labs') {
    group.traverse((child) => {
      if (!child.name.startsWith('ENVSCI__')) return;
      child.userData.districtId = definition.id;
      child.userData.featureRole ??= child.userData.exteriorProgram === true ? 'building' : 'infrastructure';
      child.userData.featureTag ??= campusFeatureKey(child.name);
    });
...
export function createDistrictModel(definition: DistrictDefinition): ProceduralModel {
  ...
  const isPharmacologyDistrict = definition.id === 'pharmacology-labs';
  ...
  const isGenomicsLabsDistrict = definition.id === 'genomics-labs';
  const isProteomicsLabsDistrict = definition.id === 'proteomics-labs';
  const isOmicsLabsDistrict = definition.id === 'omics-labs';
  const isComputationalBiologyLabsDistrict = definition.id === 'computational-biology-labs';
  const isRoboticsLabsDistrict = definition.id === 'robotics-labs';
  const isElectronicsLabsDistrict = definition.id === 'electronics-microelectronics-labs';
  const isBiochemistryLabsDistrict = definition.id === 'biochemistry-labs';
  const isOrganicChemistryLabsDistrict = definition.id === 'organic-chemistry-labs';
  const isInorganicChemistryLabsDistrict = definition.id === 'inorganic-chemistry';
  const isParticlePhysicsLabsDistrict = definition.id === 'particle-physics-labs';
  const isAstronomyAstrobiologyLabsDistrict = definition.id === 'astronomy-astrobiology-labs';
  const isMaterialsScienceLabsDistrict = definition.id === 'materials-science-lab';
  const isEnvironmentalScienceLabsDistrict = definition.id === 'environmental-science-labs';
      ...
      : isEntryLogisticsDistrict
        ...
        : isSecurityDistrict
          ...
          : isForensicCyberforensicDistrict
            ? metresToWorldUnits(0.08)
          : isGenomicsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isProteomicsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isOmicsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isComputationalBiologyLabsDistrict
            ? metresToWorldUnits(0.08)
          : isRoboticsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isElectronicsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isBiochemistryLabsDistrict
            ? metresToWorldUnits(0.08)
          : isOrganicChemistryLabsDistrict
            ? metresToWorldUnits(0.08)
          : isInorganicChemistryLabsDistrict
            ? metresToWorldUnits(0.08)
          : isParticlePhysicsLabsDistrict
            ? metresToWorldUnits(0.08)
          : isAstronomyAstrobiologyLabsDistrict
            ? metresToWorldUnits(0.08)
          : isMaterialsScienceLabsDistrict
            ? metresToWorldUnits(0.08)
          : isEnvironmentalScienceLabsDistrict
            ? metresToWorldUnits(0.08)
          : isToxicologyLabsDistrict
            ? metresToWorldUnits(0.08)
          : isResidentialEverHourDistrict
            ? metresToWorldUnits(0.08)
          : isArtMarketingDistrict
            ? metresToWorldUnits(0.08)
  ...
  const darkCenterPassageWidth = Math.max(2.1, width * 0.16);
  const plotGeometry = definition.id === 'dark-center-lab-megabuilding'
    ? (() => {
        const wingWidth = (width - darkCenterPassageWidth) * 0.5;
        const west = new THREE.BoxGeometry(wingWidth, finishedFloorY, depth);
        const east = new THREE.BoxGeometry(wingWidth, finishedFloorY, depth);
        west.translate(-(darkCenterPassageWidth + wingWidth) * 0.5, 0, 0);
        east.translate((darkCenterPassageWidth + wingWidth) * 0.5, 0, 0);
        const merged = mergeGeometries([west, east], false);
        west.dispose();
        east.dispose();
        if (!merged) throw new Error('Dark Center split foundation geometry could not be created');
        return merged;
      })()
    : new THREE.BoxGeometry(width, finishedFloorY, depth);
  const plot = prepareMesh(new THREE.Mesh(plotGeometry, plotMaterial), definition.id);
  ...
  switch (definition.category) {
    ...
    case 'bioscience':
      ...
      else if (definition.id === 'pharmacology-labs') buildPharmacologyDistrict(group, definition);
      else if (definition.id === 'microbiology-labs') buildMicrobiologyDistrict(group, definition);
      else if (definition.id === 'molecular-biology-labs') buildMolecularBiologyDistrict(group, definition);
      else if (definition.id === 'bioanalytics-lab') buildBioanalyticsLabsDistrict(group, definition);
      else if (definition.id === 'genomics-labs') buildGenomicsLabsDistrict(group, definition);
      else if (definition.id === 'proteomics-labs') buildProteomicsLabsDistrict(group, definition);
      else if (definition.id === 'omics-labs') buildOmicsLabsDistrict(group, definition);
      else if (definition.id === 'computational-biology-labs') buildComputationalBiologyLabsDistrict(group, definition);
      else if (definition.id === 'toxicology-labs') buildToxicologyLabsDistrict(group, definition);
      else buildBioscience(group, definition, height, random);
      break;
    case 'engineering':
      if (definition.id === 'robotics-labs') buildRoboticsLabsDistrict(group, definition);
      else if (definition.id === 'electronics-microelectronics-labs') buildElectronicsLabsDistrict(group, definition);
      else if (definition.id === 'materials-science-lab') buildMaterialsScienceLabsDistrict(group, definition);
      else buildEngineering(group, definition, height);
      break;
    case 'chemistry':
      if (definition.id === 'biochemistry-labs') buildBiochemistryLabsDistrict(group, definition);
      else if (definition.id === 'organic-chemistry-labs') buildOrganicChemistryLabsDistrict(group, definition);
      else if (definition.id === 'inorganic-chemistry') buildInorganicChemistryLabsDistrict(group, definition);
      else buildChemistry(group, definition, height);
      break;
    case 'physics':
      if (definition.id === 'particle-physics-labs') buildParticlePhysicsLabsDistrict(group, definition);
      else if (definition.id === 'astronomy-astrobiology-labs') buildAstronomyAstrobiologyLabsDistrict(group, definition);
      else buildPhysics(group, definition, height);
      break;
    case 'security':
    ...
    case 'commercial':
      ...
      else if (definition.id === 'even-hour-hotel') buildEverHourDistrict(group, definition);
      else if (definition.id === 'scientist-residential') buildResidentialScientistsDistrict(group, definition);
    ...
    case 'academic':
      ...
      break;
    case 'environmental':
      if (definition.id === 'environmental-science-labs') buildEnvironmentalScienceLabsDistrict(group, definition);
      else buildEnvironmental(group, definition, height);
      break;
  }

  if (!isAcademicDistrict && !isEntryLogisticsDistrict && !isSecurityDistrict && !isSecretLabsDistrict && !isMedicalLabsDistrict && !isPharmacologyDistrict && !isMicrobiologyDistrict && !isMolecularBiologyDistrict && !isBioanalyticsLabsDistrict && !isForensicCyberforensicDistrict && !isGenomicsLabsDistrict && !isProteomicsLabsDistrict && !isOmicsLabsDistrict && !isComputationalBiologyLabsDistrict && !isRoboticsLabsDistrict && !isElectronicsLabsDistrict && !isBiochemistryLabsDistrict && !isOrganicChemistryLabsDistrict && !isInorganicChemistryLabsDistrict && !isParticlePhysicsLabsDistrict && !isMaterialsScienceLabsDistrict && !isEnvironmentalScienceLabsDistrict && !isToxicologyLabsDistrict && !isResidentialEverHourDistrict && !isArtMarketingDistrict) {
  ...
  if (definition.id !== 'industrial-labs' && !isEntryLogisticsDistrict && !isSecurityDistrict && !isSecretLabsDistrict && !isMedicalLabsDistrict && !isPharmacologyDistrict && !isMicrobiologyDistrict && !isMolecularBiologyDistrict && !isBioanalyticsLabsDistrict && !isForensicCyberforensicDistrict && !isGenomicsLabsDistrict && !isProteomicsLabsDistrict && !isOmicsLabsDistrict && !isComputationalBiologyLabsDistrict && !isRoboticsLabsDistrict && !isElectronicsLabsDistrict && !isBiochemistryLabsDistrict && !isOrganicChemistryLabsDistrict && !isInorganicChemistryLabsDistrict && !isParticlePhysicsLabsDistrict && !isMaterialsScienceLabsDistrict && !isEnvironmentalScienceLabsDistrict && !isToxicologyLabsDistrict && !isResidentialEverHourDistrict && !isArtMarketingDistrict) addDistrictSignature(group, definition, height, random);
  populateDistrictSectorCampus(group, definition, random);
  if (!isEntryLogisticsDistrict && !isSecurityDistrict && !isSecretLabsDistrict && !isMedicalLabsDistrict && !isPharmacologyDistrict && !isMicrobiologyDistrict && !isMolecularBiologyDistrict && !isBioanalyticsLabsDistrict && !isForensicCyberforensicDistrict && !isGenomicsLabsDistrict && !isProteomicsLabsDistrict && !isOmicsLabsDistrict && !isComputationalBiologyLabsDistrict && !isRoboticsLabsDistrict && !isElectronicsLabsDistrict && !isBiochemistryLabsDistrict && !isOrganicChemistryLabsDistrict && !isInorganicChemistryLabsDistrict && !isParticlePhysicsLabsDistrict && !isMaterialsScienceLabsDistrict && !isEnvironmentalScienceLabsDistrict && !isToxicologyLabsDistrict && !isResidentialEverHourDistrict && !isArtMarketingDistrict) addCyberpunkDistrictLife(group, definition, height);
  ...
  return {
    ...
    labelHeight: definition.id === 'entry-commercial'
      ...
      ? 16.4
      : definition.id === 'pharmacology-labs'
      ? 12.8
      : definition.id === 'microbiology-labs'
      ? 17.2
      : definition.id === 'molecular-biology-labs'
      ? 16.8
      : definition.id === 'forensic-cyberforensic-lab'
      ? 17.6
      : definition.id === 'genomics-labs'
      ? 13.8
      : definition.id === 'omics-labs'
      ? 16.6
      : definition.id === 'robotics-labs'
      ? 18.4
      : definition.id === 'electronics-microelectronics-labs'
      ? 18.8
      : definition.id === 'biochemistry-labs'
      ? 14.6
      : definition.id === 'particle-physics-labs'
      ? 15.4
      : definition.id === 'astronomy-astrobiology-labs'
      ? 16.2
      : definition.id === 'materials-science-lab'
      ? 16.8
      : definition.id === 'environmental-science-labs'
      ? 14.8
