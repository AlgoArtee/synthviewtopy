...
export interface CygnusSkyState {
  visible: boolean;
  fixedWorldBearing: boolean;
  companionToBlackHoleDiameter: number;
  blackHole: CelestialBodyState;
  companion: CelestialBodyState;
  animation: {
    coronaPeriodSeconds: number;
    coronaPhase: number;
    coronaPulse: number;
    streamTransitSeconds: number;
    streamDirection: 'companion-to-black-hole';
  };
}

export interface CygnusSystem {
  group: THREE.Group;
  companionDirection: THREE.Vector3;
  update: (camera: THREE.Camera, time: number, underwater: boolean, visibility: number, daylight: number) => void;
  getState: (camera: THREE.Camera) => CygnusSkyState;
}
...
export function createCygnusSystem(): CygnusSystem {
  ...
  const directionAtOffset = (offset: THREE.Vector2): THREE.Vector3 => SKY_DIRECTION.clone()
  ...
  const bodyState = (camera: THREE.Camera, direction: THREE.Vector3): CelestialBodyState => {
